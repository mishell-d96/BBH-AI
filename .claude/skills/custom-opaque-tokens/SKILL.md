...
2. **CREDENTIAL/PII CHECK** — any segment decoding to user / email / password / role / tenant_id is a **standalone disclosure finding AND a chain seed** → route `/information-disclosure` + `/access-control-idor`; a live password = `/authentication` ATO.
