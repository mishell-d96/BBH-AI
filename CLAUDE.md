...
- **Online-writeup cross-check (mandatory engagement close-out step).** Before declaring an engagement done, I do **online research for public writeups/advisories/solutions of the same (or analogous) target/tech**, diff their vulnerability list against what I actually found, and for **every miss** state the **root cause** and **how I could have found it without the writeup**. Then I fold the fix back: generally-applicable → the relevant skill + recon flow; target-specific → `_RECON/<target>/notes.md`; durable behavioral → memory; and I log the iteration in `_TOOLING/beta-retrospective.md`. A clean cross-check (nothing missed) is itself recorded as a validation. The full procedure lives in **`_TOOLING/beta-mode.md`**.
- **Beta-mode trigger file.** While **`_TOOLING/beta-mode.md` is present**, BETA MODE is ACTIVE and the steps above — especially the online-writeup cross-check and fold-back — run **automatically as the engagement close-out**, without needing to be asked. Remove that file to pause beta mode.
